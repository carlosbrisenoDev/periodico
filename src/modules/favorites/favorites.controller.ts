...
const getSubscriberId = (req: AuthenticatedRequest): string | null => req.subscriber?.subscriberId ?? null;
...
export const listFavorites = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const subscriberId = getSubscriberId(req);
  if (!subscriberId) {
	res.status(401).json({ message: 'Unauthorized' });
	return;
