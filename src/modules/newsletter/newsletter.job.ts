...
import type { ArticleDoc } from '../article/article.model.js';
import type { SubscriberDoc } from '../subscribers/subscribers.model.js';
...
const getTopArticles = async (count: number): Promise<ArticleDoc[]> => {
...
const getActiveSubscribers = async (): Promise<Pick<SubscriberDoc, 'email' | 'username'>[]> =>
  SubscriberModel.find({ active: true, status: 'active' })
    .select('email username')
    .lean<Pick<SubscriberDoc, 'email' | 'username'>[]>()
    .exec();
...
const buildArticleRow = (article: ArticleDoc): string => {
...
const buildNewsletterHtml = (articles: ArticleDoc[]): string => {
...
const sendNewsletter = async (): Promise<void> => {
  ...
  process.stdout.write(
    `[Newsletter] Completed: ${sent} sent, ${failed} failed, ${articles.length} articles included.\n`,
  );
};
...
export const startNewsletterJob = (): void => {
  const intervalMs = env.NEWSLETTER_INTERVAL_MS ?? 604_800_000; // default: 7 days
  const articlesCount = env.NEWSLETTER_ARTICLES_COUNT ?? 4;

  process.stdout.write(
    `[Newsletter] Job scheduled — every ${(intervalMs / 3_600_000).toFixed(1)}h, top ${articlesCount} articles\n`,
  );
...
export const stopNewsletterJob = (): void => {
  if (intervalId !== null) {
