...
import { articleIdSchema, articleSlugSchema, createArticleSchema, listArticlesQuerySchema, updateArticleFeatureSchema, updateArticleSchema, updateArticleStatusSchema } from './article.schemas.js';
import { createArticle, duplicateArticle, deleteArticle, getArticleById, getArticleBySlug, listDeletedArticles, listArticles, publishArticleNow, purgeArticle, restoreArticle, updateArticle, updateArticleFeature, updateArticleStatus } from './article.controller.js';
const router = Router();
...
router.patch('/:id/feature', validateToken, requireRole('admin'), validateParamsSchema(articleIdSchema), validateBodySchema(updateArticleFeatureSchema), updateArticleFeature);
...
router.patch('/:id/status', validateToken, requireRole('admin'), validateParamsSchema(articleIdSchema), validateBodySchema(updateArticleStatusSchema), updateArticleStatus);
router.patch('/:id', validateToken, requireRole('admin', 'editor', 'journalist'), validateParamsSchema(articleIdSchema), validateBodySchema(updateArticleSchema), updateArticle);
